...
void HandeyeServer::initializeServices()
{
  ...
  take_sample_srv_ = this->create_service<easy_handeye2_msgs::srv::TakeSample>(
    "~/take_sample",
    std::bind(&HandeyeServer::takeSampleCallback, this, _1, _2));
  
  compute_calibration_srv_ = this->create_service<easy_handeye2_msgs::srv::ComputeCalibration>(
    "~/compute_calibration",
    std::bind(&HandeyeServer::computeCalibrationCallback, this, _1, _2));
  
  save_calibration_srv_ = this->create_service<easy_handeye2_msgs::srv::SaveCalibration>(
    "~/save_calibration",
    std::bind(&HandeyeServer::saveCalibrationCallback, this, _1, _2));
}

easy_handeye2_msgs::msg::SampleList HandeyeServer::samplesToMessage() const
{
  easy_handeye2_msgs::msg::SampleList msg;
  
  for (const auto& sample : sampler_->getSamples()) {
    msg.hand_world_samples.push_back(sample.robot_transform.transform);
    msg.camera_marker_samples.push_back(sample.optical_transform.transform);
  }
...
void HandeyeServer::takeSampleCallback(
  const std::shared_ptr<easy_handeye2_msgs::srv::TakeSample::Request> /*request*/,
  std::shared_ptr<easy_handeye2_msgs::srv::TakeSample::Response> response)
{
  response->success = sampler_->takeSample();
  response->samples = samplesToMessage();
...
void HandeyeServer::computeCalibrationCallback(
  const std::shared_ptr<easy_handeye2_msgs::srv::ComputeCalibration::Request> /*request*/,
  std::shared_ptr<easy_handeye2_msgs::srv::ComputeCalibration::Response> response)
{
  RCLCPP_INFO(this->get_logger(), "");
  RCLCPP_INFO(this->get_logger(), "============================================================");
  ...
  if (last_calibration_) {
    ...
    const auto& calib = last_calibration_;
    const auto& transform = calib->getTransform();
    
    response->calibration.eye_on_hand = calib->getParameters().eye_on_hand;
    response->calibration.robot_base_frame = calib->getParameters().robot_base_frame;
    response->calibration.robot_effector_frame = calib->getParameters().robot_effector_frame;
    response->calibration.tracking_base_frame = calib->getParameters().tracking_base_frame;
    response->calibration.tracking_marker_frame = calib->getParameters().tracking_marker_frame;
    response->calibration.transform = transform;
...
void HandeyeServer::saveCalibrationCallback(
  const std::shared_ptr<easy_handeye2_msgs::srv::SaveCalibration::Request> request,
  std::shared_ptr<easy_handeye2_msgs::srv::SaveCalibration::Response> response)
{
  if (!last_calibration_) {
    response->success = false;
    response->message = "No calibration to save. Compute calibration first.";
    return;
  }
  
  std::string filepath = request->filepath;
  if (filepath.empty()) {
    filepath = HandeyeCalibration::getDefaultFilepath(params_.name);
  }
  
  if (last_calibration_->saveToFile(filepath)) {
