...
: Node("handeye_calibration_server", options)
{
  ...
  // Initialize calibration backend (Constrained algorithm only)
  ...
  RCLCPP_INFO(this->get_logger(), "  Algorithm: Constrained (4-DOF)");
...
void HandeyeServer::loadConfigFromYaml(const std::string& config_file)
{
  try {
    ...
    if (config["grid_search"]) {
      grid_search_params_ = GridSearchParams::fromYaml(config["grid_search"]);
      RCLCPP_INFO(this->get_logger(), "Grid search params loaded:");
      RCLCPP_INFO(this->get_logger(), "  trans_range=%.3f m, trans_step=%.4f m",
        grid_search_params_.trans_range, grid_search_params_.trans_step);
      RCLCPP_INFO(this->get_logger(), "  rot_range=%.3f rad, rot_step=%.4f rad",
        grid_search_params_.rot_range, grid_search_params_.rot_step);
      RCLCPP_INFO(this->get_logger(), "  gd: max_iter=%d, lr=%.5f, delta=%.6f",
        grid_search_params_.gd_max_iterations,
        grid_search_params_.gd_learning_rate,
        grid_search_params_.gd_delta);
    }
  ...
  std::shared_ptr<easy_handeye2::srv::ComputeCalibration::Response> response)
{
  ...
  RCLCPP_INFO(this->get_logger(), "");
  RCLCPP_INFO(this->get_logger(), "[Step 3] Computing calibration (Constrained, 4-DOF)...");
  RCLCPP_INFO(this->get_logger(), "  Free parameters: %d", constraints_.getNumFreeParams());
  ...
  last_calibration_ = calibration_backend_->computeConstrainedCalibration(
    params_, sampler_->getSamples(), constraints_, grid_search_params_);
