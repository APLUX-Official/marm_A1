...
private:
    ...
    get_move_group(const std::string &arm_name)
    {
        ...
        move_group->setMaxVelocityScalingFactor(0.7);     // 设置最大速度缩放因子
        move_group->setMaxAccelerationScalingFactor(0.3); // 设置最大加速度缩放因子
        return move_group;
    }
        ...
        std::string &message)
    {
        ...
        // ===== 修改：增加规划时间和尝试次数 =====
        move_group->setPlanningTime(1.0);      // 增加到15秒
        move_group->setNumPlanningAttempts(2); // 增加尝试次数
            ...
            response)
    {
        ...
        try
        {
            ...
            // ===== 新增：获取机械臂工作空间信息 =====
            auto joint_names = move_group->getJointNames();
            RCLCPP_INFO(this->get_logger(), "机械臂关节数量: %zu", joint_names.size());
            for (size_t i = 0; i < joint_names.size(); ++i) {
                RCLCPP_INFO(this->get_logger(), "关节%zu: %s", i+1, joint_names[i].c_str());
            }

            // ===== 修改：设置更宽松的目标容差以提高成功率 =====
            move_group->setGoalPositionTolerance(0.08);    // 增加到8cm
            move_group->setGoalOrientationTolerance(0.3);  // 增加到约17度

            // ===== 新增：增加规划尝试次数 =====
            move_group->setNumPlanningAttempts(10); // 增加规划尝试次数
            move_group->setPlanningTime(2.0);      // 增加规划时间限制
            ...
            else
            {
                ...
                // // ===== 新增：目标位姿可达性检查 =====
                // RCLCPP_INFO(this->get_logger(), "检查目标位姿可达性...");

                // // ===== 新增：尝试逆运动学求解验证目标位姿 =====
                // robot_state::RobotState goal_state(move_group->getRobotModel());
                // goal_state.setFromIK(move_group->getJointModelGroup(request->arm_name), offset_target_pose);
                
                // if (!goal_state.satisfiesBounds()) {
                //     RCLCPP_ERROR(this->get_logger(), "目标位姿的逆运动学解超出关节限制");
                //     response->arm_status = false;
                //     response->message = "目标位姿的逆运动学解超出关节限制";
                //     return;
                // } else {
                //     RCLCPP_INFO(this->get_logger(), "逆运动学验证通过");
                // }

                // 先尝试设置目标位姿
                bool pose_valid = move_group->setPoseTarget(offset_target_pose);
                if (!pose_valid)
                {
                    response->arm_status = false;
                    response->message = "目标位姿无效或不可达";
                    RCLCPP_ERROR(this->get_logger(), "%s", response->message.c_str());
                    return;
                }
                ...
                {
                    RCLCPP_INFO(this->get_logger(), "规划尝试 %d/3", attempt);

                    if (plan_and_execute(move_group, planning_message))
                    {
                    ...
                    {
                        RCLCPP_WARN(this->get_logger(), "第%d次规划失败: %s", attempt, planning_message.c_str());
